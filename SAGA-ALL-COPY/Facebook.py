...
#replace window to windowtwit, windowfb, windowyt
window = Tk()

window.title('SAGA: Facebook')
...
image_image_1 = PhotoImage(
    file=relative_to_assets("image_1_fb.png"))
...
image_image_4 = PhotoImage(
    file=relative_to_assets("image_4_fb.png"))
...
entry_2.insert(0, "Insert Facebook Link")
...
def on_click2(event):
    if entry_2.get() == "Insert Facebook Link":
    ...
    try:
        ...
        if link == 'Insert Facebook Link':
            ...
            if respondsearch == 'yes':
                for post in get_posts(post_urls=[link], options={"comments": True, "comments": limit_int, "progress": True}):
                    for comment in post["comments_full"]:
                        fbcomments = (comment["comment_text"])
                        fb_retrieved.append(fbcomments)

                for x in fb_retrieved:
                    ...
                    fb_cleanedlist.append(i)

                    for i in range(len(fb_cleanedlist)):
                        fb_cleanedlist[i] = fb_cleanedlist[i].lower()

                    header = ['Documents']
                    with open('CollectedFbComments.csv','w', encoding='UTF8', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow(header)
                        for word in range(len(fb_cleanedlist)):
                            writer.writerow([fb_cleanedlist[word]])

                    print (fb_cleanedlist)
                global dontupdate
...
def calculating_naive():
    ...
    sentiment_final = []
    testing = []
    with open('CollectedFbComments.csv', 'r', encoding="UTF8") as csv_file:
        csv_read = csv.reader(csv_file, delimiter=',')
        for row in csv_read:
            if row[0] == "Documents":
                print ("Hello")
...
def piechart(sentiment_final):
    ...
    print (positive_forpie)
    print (negative_forpie)
    print (neutral_forpie)
...
def appendingsentiment(sentiment_final):
    ...
    with open('CollectedFbComments.csv', 'r') as read_object, \
        open('FbWithSentiment.csv', 'w', newline='') as write_object:
...
def showingtable():
    ...
    with open('FbWithSentiment.csv') as f:
...
def SaveTo():
    ...
    if dontupdate == "dontupdate":
        ...
        collectedfb_csv = fb_cleanedlist
...
def update():
    ...
    if dontupdate == 'dontupdate':
        ...
        if updatingdataset == "yes":
            ...
            Collected = pd.read_csv ('FBWithSentiment.csv', encoding='latin')
