...
#replace window to , windowfb, windowyt
window = Tk()

window.title('SAGA: Twitter')
...
entry_2.insert(0, "Search by Keyword")
...
def on_click2(event):
    if entry_2.get() == "Search by Keyword":
...
d_since=tkinter.StringVar()
d_until=tkinter.StringVar()
...
def retrieve_input():
    ...
    date_since = d_since.get()
    date_until = d_until.get()
    ...
    for x in date_since:
        if x == '/':
            con_since = date_since.replace('/', '-')

    for y in date_until:
        if y == '/':
            con_until = date_until.replace('/', '-')

    global respondsearch

    try:
        datetime.datetime.strptime(con_since, '%Y-%m-%d')
        datetime.datetime.strptime(con_until, '%Y-%m-%d')
        ...
        if keyword == 'Search by Keyword':
            ...
            respondsearch = 'no'
        elif keyword.replace(" ", "").isalpha() == False:
            messagebox.showinfo('SAGA', 'Only accepts alpha keywords.')
            respondsearch = 'no'
            ...
            limit_int = (int(limit))
            respondsearch = messagebox.askquestion('SAGA', 'Keyword: ' + keyword + '\n'
                                                + 'Count: ' + str(limit_int) + '\n'
                                                + 'Since: ' + con_since + '\n'
                                                + 'Until: ' + con_until + '\n'
            ...
            if respondsearch == 'yes':
                ...
                c.Store_object = True
                c.Pandas = True
                c.Geo = "14.599512, 120.984222,1850km"
                c.Lang = "tl"
                c.Count = True
                c.Stats = True
                twint.run.Search(c)
                global one, two
                one = twint.output.panda.Tweets_df['tweet']
                two = twint.output.panda.Tweets_df['date']
                global dontupdate
...
def converting_text():
    ...
    collected_tweet = one.values.tolist() #a nested list
    collected_dates = two.values.tolist()

    global cleaned
    cleaned = []
    for x in collected_tweet:
        a = re.sub("@[A-Za-z0-9_]+","", x)     #remove mentions with @
        b = re.sub("#[A-Za-z0-9_]+","", a)      #remove hastags
        c = re.sub(r"\S*https?:\S*", "", b) #remove https
        demize = emoji.demojize(c)          #remove emoji
        d = demize.replace(':', ' ')
        
        f = d.replace(".", '')          #remove period
        g = re.sub(r'\w*\d\w*', '', f).strip()   # remove anything with numbers
        h = re.sub("[^a-zA-Z0-9]+", " ", g)
        i = re.sub(r'\b\w{1,2}\b', '', h)    #remove anything less than 2 charac
        j = i.replace("Don't", "do not")
        k = j.replace("don't", "do not")
        e = k.replace("'", '')          #remove apostrophe
        cleaned.append(e)
    ...
    with open('CollectedTweets.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)

        for document in range(len(collected_tweet)):
            writer.writerow([cleaned[document], collected_dates[document]])



def calculating_naive(SelectedDataSet):
    ...
    sentiment_final = []
    testing = []
    with open('CollectedTweets.csv', 'r', encoding="UTF8") as csv_file:
        csv_read = csv.reader(csv_file, delimiter=',')
        for row in csv_read:
            if row[0] == "Documents":
...
def countingcollectedtweets():

    sentiments_gathered = 0 - 1
    #this for sentiments gathered
    with open('CollectedTweets.csv', 'r', encoding="UTF8") as csv_file:
        reader = csv.reader(csv_file, delimiter=',')
        for row in reader:
...
def piechart(sentiment_final):
    ...
    canvas.create_window(77, 266,  anchor=NW, window=frm)


def appendingsentiment(sentiment_final):

    with open('CollectedTweets.csv', 'r') as read_object, \
        open('WithSentiment.csv', 'w', newline='') as write_object:
...
def showingtable():
    ...
    TableMargin.pack()
    canvas.create_window(377, 266,  anchor=NW, window=TableMargin)
    ...
    tree = ttk.Treeview (
        TableMargin,
        columns=("Documents", "Dates", "Sentiments"),
        height=21,
        selectmode="extended",
    ...
    tree.heading('Dates', text="Dates", anchor=CENTER)
    ...
    tree.column('#2', stretch=FALSE, minwidth=20, width=60, anchor=W)
    tree.column('#3', stretch=FALSE, minwidth=20, width=60, anchor=W)
    tree.pack(expand=True,fill=BOTH)

    #code to open the CSV na na-analyzed na
    with open('WithSentiment.csv') as f:
        ...
        for row in reader:
            ...
            tree.insert("", 1, values=(Documents, Dates, Sentiments))
    #End of table form code

def wordcloud():
    words_collectedtweets = []
    ...
    with open('CollectedTweets.csv', 'r', encoding="UTF8") as csv_file:
        ...
        for row in reader:
            if row[0] != " ":                            #remove the header, what to do?
                csv_words = row[0].split(" ")
                for x in csv_words:
                    words_collectedtweets.append(x)


    unique_string=(" ").join(words_collectedtweets)
    ...
    canvas.create_window(964, 266,  anchor=NW, window=imagecontainer)
...
def clearall():
    ...
    entry_2.insert(0, "Search by Keyword")
    v0.set(1)
    cal_since.set_date(date.today() + timedelta(days=-1))
    cal_until.set_date(date.today())
...
def SaveTo():
    ...
    if dontupdate == "dontupdate":
        ...
        collectedtwt_csv = cleaned
        categories_csv = sentiment_final
        ...
        if file:
            ...
            with open(file,"w", encoding='UTF8', newline='') as f:
                ...
                for document in range(len(collectedtwt_csv)):
                    writer.writerow([collectedtwt_csv[document], categories_csv[document]])
...
def update():
    ...
    if dontupdate == 'dontupdate':
        ...
        if updatingdataset == "yes":
            ...
            Collected = pd.read_csv ('WithSentiment.csv', usecols=[0,2], encoding='latin')
...
def main():
    ...
    if respondsearch == 'yes':
        converting_text()
        calculating_naive(SelectedDataSet)
        wordcloud()
        sentiment_final = calculating_naive(SelectedDataSet)
        piechart(sentiment_final)
        countingcollectedtweets()
...
canvas.create_window(1143, 676,  anchor=NW, window=button_2)
...
canvas.create_window(980, 676,  anchor=NW, window=button_3)
...
cal_since = DateEntry(canvas, selectmode='day', date_pattern='y/mm/dd', maxdate=date.today() + timedelta(days=-1), textvariable=d_since, width=15, font="Arial 10 bold")
cal_until = DateEntry(canvas, selectmode='day', date_pattern='y/mm/dd', maxdate=date.today(), textvariable=d_until, width=15, font="Arial 10 bold")
cal_since.set_date(date.today() + timedelta(days=-1))

#Code for date picker to stick(be part) to canvas
canvas.create_window(274, 195, anchor=NW, window=cal_since)
canvas.create_window(469, 195, anchor=NW, window=cal_until)
